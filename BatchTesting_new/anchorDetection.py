...
class OMRProcessor:
    ...
    def detect_anchor_points(self):
        ...
        # If no contour found, try computing M_transform from anchor centers (fallback)
        if sheet_approx_poly is None:
            print("⚠️ No sheet polygon found. Attempting fallback M_transform using anchor positions...")
            fallback_M = self._compute_fallback_m_transform_from_anchors(detected_anchors=[])
            if fallback_M is not None:
                self.M_transform = fallback_M
                print("✅ Fallback M_transform set.")
        ...
        # If M_transform wasn't computed earlier, try computing it now using detected anchors
        if self.M_transform is None:
            print("⚠️ Attempting fallback M_transform using anchor centers...")
            fallback_M = self._compute_fallback_m_transform_from_anchors(detected_anchors)
            if fallback_M is not None:
                self.M_transform = fallback_M
                print("✅ Fallback M_transform successfully computed.")
            else:
                print("❌ Fallback M_transform could not be computed.")


        return detected_anchors, self.image, self.M_transform # Return deskewed image and M_transform
    
    def _compute_fallback_m_transform_from_anchors(self, detected_anchors):
        if len(detected_anchors) < 4:
            print("⚠️ Not enough anchors for fallback transformation.")
            return None

        # Create a mapping: class_name -> center
        anchor_map = {a['class_name']: a['center'] for a in detected_anchors}

        # Ensure all 4 anchors exist
        required = ['anchor_1', 'anchor_2', 'anchor_3', 'anchor_4']
        if not all(k in anchor_map for k in required):
            print("⚠️ Missing some anchor centers for fallback transformation.")
            return None

        # Source: detected anchor centers
        src_pts = np.float32([
            anchor_map['anchor_1'],
            anchor_map['anchor_2'],
            anchor_map['anchor_3'],
        ...
        # Destination: ideal positions in a rectangle layout (same order)
        ...
        print("✅ Fallback M_transform computed from anchors.")
        return M
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
            ...
            try:
                ...
                if M_transform_result is None:
                    print(f"❌ M_transform not computed for {filename}")
                else:
                    print(f"✅ M_transform computed for {filename}")

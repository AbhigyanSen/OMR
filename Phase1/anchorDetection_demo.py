...
class OMRProcessor:
    ...
    def detect_anchor_points(self):
        ...
        Returns:
            list: A list of dictionaries, each containing 'class_name', 'bbox', 'center', and 'area'.
        """
        detected_anchors = []
        ...
        for class_name_str in anchor_class_names:
            ...
            if class_id != -1 and class_id in self.annotations:
                for original_bbox_coords in self.annotations[class_id]: # Use original annotations from the loaded file
                    ...
                    x1, y1, x2, y2 = current_bbox_coords
    ...
    def visualize_results(self, detected_anchors, output_filename="detected_anchors_output.jpg"):
        """
    ...
    def process_folder(self, folder_path, output_folder="output"):
        """
        Process all images in a folder and save results.
        Args:
            folder_path (str): Path to the folder containing images.
            output_folder (str): Output folder name to save results.
        """
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png'):
                ...
                output_filename = os.path.join(output_folder, f"output_{os.path.basename(folder_path)}", filename)
                
                # Instantiate OMRProcessor for each image
                processor = OMRProcessor(image_path, self.annotations_path, self.classes_path)
                detected_anchors = processor.detect_anchor_points()
                processor.visualize_results(detected_anchors, output_filename)

                # Print detected anchor details if needed
                print(f"Processed {filename}. Results saved to {output_filename}")

...
if __name__ == "__main__":
    ...
    # IMPORTANT: Use the annotation file that corresponds to the specific layout of your OMR sheets.
    # If the layout (and thus anchor positions) changes per image, you might need dynamic annotation loading.
    annotations_file = r"D:\Projects\OMR\new_abhigyan\Phase1\Data\labels\BE23-01-01003.txt"  
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
            ...
            try:
                # Instantiate OMRProcessor for EACH image to ensure proper loading and deskewing
                processor = OMRProcessor(image_path, annotations_file, classes_file)
                
                # detect_anchor_points now returns deskewed_img and M_transform
                ...
                if len(detected_anchors) < 4:
                    ...
                    # Save the deskewed image if deskewing occurred, otherwise the original
                    cv2.imwrite(warning_path, deskewed_img_result if deskewed_img_result is not None else processor.original_image)
                    # For images with missing anchors, we might want to still record what was found, or record 'None'
                    ...
                    continue  # Skip visualization for warnings, but data is recorded

                else:
                    output_image_path = os.path.join(output_dir, filename)
                    # visualize_results now returns the dictionary of anchor centers
                    ...
                    # Store anchor data for the current image
                    all_image_anchor_data[filename] = {
                        "anchors": anchor_centers_for_json, # Already in desired format
                        "M_transform": M_transform_result.tolist() if M_transform_result is not None else None, # Convert to list for JSON
                        "deskewed_width": processor.deskewed_width,
                        "deskewed_height": processor.deskewed_height
                    }
                ...
                try:
                    ...
                    # Store partial/error data for the current image
                    all_image_anchor_data[filename] = {
                        "anchors": {}, # No anchors detected due to error
                        "M_transform": None,
                        "deskewed_width": None,

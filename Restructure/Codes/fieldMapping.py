...
class OMRFieldMapper:
    def __init__(self, annotated_image_path, annotations_file, classes_path, 
        ...
        Args:
            annotated_image_path (str): Path to the reference annotated image (e.g., BE24-05-01001.jpg).
            annotations_file (str): Path to the reference YOLO annotation file (e.g., BE24-05-01001.txt).
    ...
    def save_cropped_fields(self, deskewed_image, mapped_fields, output_base_dir, image_filename, target_field_names):
        """
        Saves cropped regions of specified fields from the deskewed image.
        
        :param deskewed_image: The deskewed full image
        :param mapped_fields: Dictionary containing bbox data from map_fields_and_visualize
        :param output_base_dir: Base output directory (e.g., "./ICR")
        :param image_filename: Original image filename to use for naming
        :param target_field_names: List of field names to extract (e.g., ['reg_no', 'roll_no'])
        """
        for key, data in mapped_fields.items():
            # Match exact field name, or prefix in case of multiple boxes (e.g., reg_no_0)
            base_field_name = key.split('_')[0] if '_' in key else key
            if base_field_name in target_field_names or key in target_field_names:
                x1, y1, x2, y2 = data["bbox"]
                cropped = deskewed_image[y1:y2, x1:x2]

                # Construct output path
                field_folder = os.path.join(output_base_dir, base_field_name)

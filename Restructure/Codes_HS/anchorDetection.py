...
def get_annotation_files(annotations_dir):
    """
    Given the path to the annotation folder (Annotations),
    finds:
      - single annotated image
      - single label file
      - classes.txt file
    Ensures image and label filenames match (excluding extension).

    Args:
        annotations_dir (str): Path to annotation folder

    Returns:
        tuple: (annotated_image_path, annotations_file, classes_file)

    Raises:
        SystemExit: If any error occurs (multiple files or mismatched names)
    """
...
if __name__ == "__main__":
    ...
    # annotations_file = os.path.join(base_folder, "Annotations", omr_template_name, "labels", "TEST-01003.txt")
    # classes_file = os.path.join(base_folder, "Annotations", omr_template_name, "classes.txt")
    # annotated_image_path = os.path.join(base_folder, "Annotations", omr_template_name, "images", "TEST-01003.jpg")

...
def detect_marked_options(mapped_json_path, processed_images_folder,
    ...
    logger.info("Starting marked options detection...")
    ...
    for img_name, annotations in mapped_data.items():
        ...
        for label, data in fields.items():
            if not isinstance(data, dict) or "bbox" not in data:
                logger.debug(f"Skipping invalid field: {label}")
                continue
            bbox = data["bbox"]
        ...
        for qid in sorted(question_groups.keys(), key=lambda x: int(x[1:])):
            ...
            if marked_label:
                ...
                # logger.info(f"{img_name}: {qid} -> {marked_label}")
        ...
        for key_name, display_name in key_fields.items():
            ...
            # logger.info(f"{img_name}: {key_name} -> {number_value}")
    ...
    logger.info(f"Flat mapping JSON saved to: {output_text_json_path}")
    logger.info("Marked options detection completed.")
...
def process_marked_options(base_folder, omr_template_name, date, batch_name):
    ...
    mapped_json_path = os.path.join(
        base_folder, "Images", omr_template_name, date, "Output", batch_name, "annotate_" + batch_name, "field_mappings.json"
    )
    processed_images_folder = os.path.join(
        base_folder, "Images", omr_template_name, date, "Output", batch_name, f"raw_{batch_name}"
    )
    ...
    logger.info("Generating generalized JSON...")
    generate_generalized_json(base_json_path, edge_json_path, verification_csv_path, generalized_json_path, key_fields_json, classes_file, logger)

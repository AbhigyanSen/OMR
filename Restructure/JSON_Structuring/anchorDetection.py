...
def generate_generalized_json(base_folder, folder_path, all_image_anchor_data, warning_dir):
    ...
    template_name = os.path.basename(os.path.dirname(folder_path))  # e.g., "TestData"
    batch_name = os.path.basename(folder_path)                      # e.g., "BE24-05-01"
    process_dt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    date_str = datetime.now().strftime("%d%m%Y")                    # ⬅️ Only date, no time
    ...
    for image_name, data in all_image_anchor_data.items():
        ...
        # Final IMAGENAME format: \HSOMR\DDMMYYYY\INPUT\BATCHNAME\ImageName
        custom_image_name = f"\\HSOMR\\{date_str}\\INPUT\\{batch_name}\\{image_name}"

        image_entry = {
            "IMAGENAME": custom_image_name,
            "IMAGEPATH": image_abs_path.replace("/", "\\"),  # absolute path
            "SEQ": seq,
            "SKEWED": skewed,
